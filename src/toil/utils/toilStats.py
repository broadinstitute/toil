...
def buildElement(element, items, itemName):
    ...
    def extractAndRound(name, item):
        valueToRound = item.get(name, 0)
        return __round(float(valueToRound))
    ...
    itemTimes = [ extractAndRound("time", item) for item in items ]
    itemTimes = filterAndSort(itemTimes)
    itemClocks = [ extractAndRound("clock", item) for item in items ]
    itemClocks = filterAndSort(itemClocks)
    itemWaits=[]
    for index in range(0,len(itemTimes)):
        itemWaits.append(itemClocks[index]-itemTimes[index])
    itemWaits = filterAndSort(itemWaits)
    itemMemory = [ extractAndRound("memory",item) for item in items ]
    itemMemory = filterAndSort(itemMemory)
...
def createSummary(element, containingItems, containingItemName, getFn):
    ...
    if len(itemCounts) == 0:
        itemCounts.append(0)
    element["median_number_per_%s" %
                   containingItemName] = itemCounts[len(itemCounts) / 2]
    element["average_number_per_%s" %
                   containingItemName] = float(sum(itemCounts)) / len(itemCounts)
    element["min_number_per_%s" %
                   containingItemName] = min(itemCounts)
    element["max_number_per_%s" %
                   containingItemName] = max(itemCounts)
...
def processData(config, stats, options):
    ...
    ##########################################
    if stats.get("total_time", None) == None:  # Hack to allow unfinished toils.
...
def main():
    """ Reports stats on the toil run, use with --stats option to toil.
    """
